...
print(config.core_catalog)     # e.g., '{{ cookiecutter.core_catalog }}' (same across all environments)
```
...
# Returns: "{{ cookiecutter.core_catalog }}.sportsbook_retail.gameplay_legs" (in all environments)
```
...
- `bundle.core_catalog` - The catalog for core source tables (e.g., '{{ cookiecutter.core_catalog }}', same across all environments)
